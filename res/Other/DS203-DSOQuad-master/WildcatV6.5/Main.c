...
//----------------------------------------------------------------------------------------------------------------
      if(Key_Buffer == K_INDEX_DEC){                                            // ===--- TASTO 5 --- PREMUTO --====
         ...
         if((Current==T_BASE)&&(_Det==MODE)&&(FPGAver>0)&&(AutoTrigSelect)){
           if(FreeRunEnable>0)FreeRunEnable--;
           goto BypassUpdate;
         }

         if((OffsetSelect==1)&&(Current>1)&&(!((Current==T_BASE)&&(_Det==1)&&(FPGAsubVer)&&(FPGAosFlag)))){
           if(ADCoffset>0)ADCoffset--;
           Update_Range();
           goto bypasslongpress;
         }

         if((OffsetSelect==2)&&(Current>1)&&(!((Current==T_BASE)&&(_Det==1)&&(FPGAsubVer)&&(FPGAosFlag)))){
         ...
         if((OS_RateSelect)&&(Current==T_BASE)&&(_Det==1)&&(OSBufferLogic())
           &&(FPGAsubVer)&&(FPGAosFlag)){                                        //Tbase RANGE sub menu
             if(OS_Range<5)OS_Range++;
             Update_Base();
             goto BypassUpdate;
         }
	    ...
	    if((FrameMode<2)&&(Current == T_BASE)&&(_Det == BASE)){                 
          ...
          if(Current == FILE){                                          //changing file extensions
            if(_Curr[0].Value == LOAD){                                              
              //if(_Curr[2].Value == CSV){_Curr[2].Value = BUF;ResetEditList();}       
              //if(_Curr[2].Value ==  7 ){_Curr[2].Value =  6 ;ResetEditList();}       
            }
...
NoDec:
     ...
     while (Delay_Cnt > 1000){
       if((__Get(KEY_STATUS)& K_ITEM_S_STATUS)!=0){
         ...
         if(EditListActive(0)){                                                                   //reset hilighted char to space
          ...
          if(Current != FILE){
           if(_Mode==X_Y)goto bypasslongpress;		//don't change buffer size this way in xy mode
            ...
            if (FlagFrameMode==0){                      //entering change from full buffer mode to single window
              ...
              OSAvg=0;
              FPGAosFlag=0;
            }else if((OSBuffer==1)&&(OSAvg==0)&&(FPGAsubVer)&&(FPGAosFlag==0)){
              FPGAosFlag=1;
              Update_Base();
...
//=============================================================================================================================
    ...
    while ((Delay_Cnt >0)&&(Delay_Cnt <=1000)){					// long press
     ...
     if((__Get(KEY_STATUS)& K_ITEM_S_STATUS)!=0){
       ...
       if(EditListActive(0)){                                                     //reset all hilighted chars to space
...
bypasslongpress:
     if((Key_Buffer!=K_INDEX_DEC)&&(Key_Buffer!=K_INDEX_INC)){OffsetSelect=0;OS_RateSelect=0;}  //reset on all buttons except adjusts
bypassOSselect:
...
{
  u8 i;
            if(UartLogic()==0){       //enabling this causes T cursors to be moved to the left of meters when they are engaged
...
void Config_init(void){
  ...
  if(OSBufferLogic()){
    InitiateOSBuffers();
    if((FPGAsubVer)&&(FPGAosFlag))_X_posi.Value=GetXpos(XposRef);         
  }
...
u8 i;
                  ...
                  if((FPGAver>0)&&(Current==T_BASE)&&(_Det==MODE)&&(_Mode==AUTO)){                             
                    if(AutoTrigSelect)AutoTrigSelect=0;else AutoTrigSelect=1;
                    return 1;
                  }              
